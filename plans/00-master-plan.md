...
This delivery implements that first increment as real, running code. Scope: **ORD-01 (create) + ORD-02 (query) + ORD-04 (status transition)**, Publish-Subscribe to the existing webhook machinery, timeout+bounded-retry resilience, new `docker-compose.yml` (none exists today — creating one doesn't violate `CLAUD.md`'s "don't modify docker-compose.yml", nothing to modify), and a Terraform plan to provision two EC2 instances + RDS on AWS Academy Learner Lab (no Fargate — Learner Lab blocks custom IAM roles).

**ORD-01's total calculation stays a Django responsibility** — tax/discount/shipping pricing (`checkout/calculations.py`, plugin manager) is not migrated in this increment; `order-service` trusts the totals Django computes and sends, and only checks them for internal consistency (non-negative, net ≤ gross) rather than deriving them independently. Recomputing pricing inside `order-service` would mean porting a large chunk of the monolith's tax/discount engine — out of scope for this increment.

**ORD-04's State pattern** (`order_service/app/domain/order_status.py`) is wired to a real endpoint: `PATCH /orders/{id}/status`, returns 422 via `InvalidTransitionError` on an invalid transition, 404 if the order doesn't exist. Not exposed through GraphQL (no monolith-side mutation calls it yet) — verified directly against the REST API.
